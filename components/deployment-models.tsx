...
type DeploymentModelsProps = {
  /** Match parent SectionChapter so the Orb background fits light vs dark surfaces */
  chapterSurface?: "light" | "dark";
};

export function DeploymentModels({ chapterSurface = "dark" }: DeploymentModelsProps) {
  const orbBg = chapterSurface === "light" ? "#f4f4f5" : "#000000";
  const orbOpacity = chapterSurface === "light" ? "opacity-35" : "opacity-50";

  return (
    <section className="relative py-24 border-t border-border overflow-hidden min-h-[600px]">
      <div
        className={`absolute inset-0 z-0 w-full h-full min-h-[600px] ${orbOpacity}`}
        aria-hidden
      >
        <Orb
          hoverIntensity={2}
